...
def generate_ip_addresses(ip_range):
    ip_addresses = [str(ip) for ip in range(ipaddress.IPv4Network(ip_range, strict=False).network_address,
                                           ipaddress.IPv4Network(ip_range, strict=False).broadcast_address + 1)]
    return ip_addresses
